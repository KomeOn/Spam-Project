...
def rfc_classifier(files, var):
    ...
    if int(var) == 1:
        ...
        d_test = dsets['text']         
        model_rf = load(open('RFC.pkl', 'rb'))
        vect = load(open('vectRFC.pkl', 'rb'))
        tf = TfidfTransformer()
...
def bagging_classifier(files, var):
    ...
    if int(var) == 1:
        ...
        print("Inside testing phase : ")
        d_test = dsets['text']         
        bag_class = load(open('BAG.pkl', 'rb'))
        vect = load(open('vectBAG.pkl', 'rb'))
        tf = TfidfTransformer()
        load_vect = CountVectorizer(vocabulary=vect)
        last = len(d_test)
        for i in range(0, last):
...
def adaB_classifier(files, var):
    ...
    if int(var) == 1:
        ...
        d_test = dsets['text']       
        ada_class = load(open('ADA.pkl', 'rb'))
        vect = load(open('vectADA.pkl', 'rb'))
        tf = TfidfTransformer()
        load_vect = CountVectorizer(vocabulary=vect)
        last = len(d_test)
        for i in range(0, last):
