...
def RFC_class(files):
    ...
    df = pd.DataFrame.from_items(pred_scores,orient='index', columns=['N_Trees', 'Score'])
    df.plot(figsize=(11,6))
    print("RFC : \n",df[df['Score'] == df['Score'].max()])
    sbst = df[df['Score'] == df['Score'].max()].head(1)
    sbst_id = sbst.first_valid_index()
    nt = sbst.get_value(sbst_id, 'N_Trees')
    score = sbst.get_value(sbst_id,'Score')
    best_ser.update({
                'n_trees' : nt,
                'score' : score
            })
    max_dict['Random Forest Classifier'] = best_ser
    return best_ser
...
def AB_class(files):
    ...
    df = pd.DataFrame.from_items(pred_scores,orient='index', columns=['N_Classifiers','Score'])
    df.plot(figsize=(11,6))
    print("AB : \n",df[df['Score'] == df['Score'].max()])
    sbst = df[df['Score'] == df['Score'].max()].head(1)
    sbst_id = sbst.first_valid_index()
    nc = sbst.get_value(sbst_id, 'N_Classifiers')
    score = sbst.get_value(sbst_id,'Score')
    best_ser.update({
                'n_classifiers' : nc,
                'score' : score
            })
    max_dict['AdaBoosting Classifier'] = best_ser
    return best_ser
...
def BAG_class(files):
    ...
    max_dict['Bagging Classifiers'] = best_ser
    return best_ser
...
def menu(files):
    ...
    print("\n\nEnsemble Classifiers Hyperparameter tuning......")
