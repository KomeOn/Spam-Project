...
def Multi_NB(files, var):
    ...
    if int(var) == 1:
        ...
        d_test = dsets['text']    
        modelMNB = load(open('pickle_files/MNB.pkl', 'rb'))
        vect = load(open('pickle_files/vectMNB.pkl', 'rb'))
        tf = TfidfTransformer()
...
def SVM(files, var):
    ...
    if int(var) == 1:
        ...
        print("Inside testing phase : ")
        d_test = dsets['text']   
        model_svm = load(open('pickle_files/SVM.pkl', 'rb'))
        vect = load(open('pickle_files/vectSVM.pkl', 'rb'))
        tf = TfidfTransformer()
        load_vect = CountVectorizer(vocabulary=vect)
        last = len(d_test)
        for i in range(0, last):
...
def D_Tree(files, var):
    ...
    if int(var) == 1:
        ...
        print("Inside testing phase : ")
        d_test = dsets['text']
        model_dtree = load(open('pickle_files/DecT.pkl', 'rb'))
        vect = load(open('pickle_files/vectDecT.pkl', 'rb'))
        tf = TfidfTransformer()
        load_vect = CountVectorizer(vocabulary=vect)
        last = len(d_test)
        for i in range(0, last):
...
def KNC(files, var):
    ...
    if int(var) == 1:
        ...
        print("Inside testing phase : ")
        d_test = dsets['text']         
        knc = load(open('pickle_files/KNC.pkl', 'rb'))
        vect = load(open('pickle_files/vectKNC.pkl', 'rb'))
        tf = TfidfTransformer()
        load_vect = CountVectorizer(vocabulary=vect)
        last = len(d_test)
        for i in range(0, last):
